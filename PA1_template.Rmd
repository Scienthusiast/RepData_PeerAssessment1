...
```{r loading}
    setwd("/Users/timothee/Documents/Serendipity/dataAnalysis/coursera reproducible research/assessment 1/RepData_PeerAssessment1") # Must be replaced with an adequate path -  That cannot be reproducible... what do ?
...
The median number of steps in a day is `r medianStepsByDay` steps, the mean number of steps in a day is `r meanStepsByDay` steps.
...
```{r dailyPattern}
    ...
    plot(intervals, stepsByInt, type = "l", main = "Number of steps by interval", xlab="Intervals", ylab ="Number of steps")
    maxStepsInt <- intervals[which(stepsByInt==max(stepsByInt))]
```

The time interval with the maximum mean number of steps in a day is the interval labeled `r maxStepsInt`.
...
```
There are `r nbMissingIntervals` missing values of the number of steps for a time interval, and there are `r nbMissingDays` days with incomplete or no data.  
...
```{r NAFilledIn}
    ...
    hist(stepsByDayNoNA, breaks = 8, main = "Histogram of number of steps by day", xlab = "Number of steps in a day")
    meanStepsByDayNoNA <- mean(stepsByDayNoNA, na.rm = T)
    medianStepsByDayNoNA <- median(stepsByDayNoNA, na.rm = T)
```
Now, with the NAs filled in, the median number of steps in a day is `r medianStepsByDayNoNA` steps, the mean number of steps in a day is `r meanStepsByDayNoNA` steps.  

With the strategy used for imputing missing data, the mean number of steps doesn't change. The median, however, changes from `r medianStepsByDay`to `r medianStepsByDayNoNA`, and could change quite dramatically as we input many times the exact same values for days with no data.
...
```{r weekDays}
    ...
    weekday <- factor( weekdays(as.Date(activity$date)) %in% c("Saturday","Sunday"), labels = c("weekday", "weekend"))
